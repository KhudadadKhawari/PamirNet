  ...
  return (
    <AppShell
      ...
      onExitImpersonation={async () => {
        await request<void>("/platform/impersonation/stop/", { method: "POST" }, access);
        await refresh();
      }}
      onLogout={async () => {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-100 px-4 text-slate-900">
      <form onSubmit={submit} className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">ISP Operations</p>
        ...
        <button className="w-full rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" disabled={submitting}>
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      {me.impersonating && (
        <div className="flex items-center justify-between bg-amber-100 px-4 py-2 text-sm text-amber-950">
          ...
          <button className="font-semibold underline" onClick={onExitImpersonation}>Exit impersonation</button>
        </div>
      )}
      <div className="flex min-h-screen">
        <aside className="hidden w-56 border-r border-slate-200 bg-slate-900 text-slate-100 md:block">
          ...
          <nav className="p-3">
            {navigation.map((item, index) => (
              <button key={item} className={`mb-1 w-full rounded px-3 py-2 text-left text-sm ${index === 0 ? "bg-slate-800" : "text-slate-300 hover:bg-slate-800"}`}>
                {item}
              </button>
            ))}
          </nav>
        ...
        <main className="min-w-0 flex-1">
          <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-3">
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <button className="rounded border border-slate-300 px-3 py-1.5" onClick={onLogout}>Sign out</button>
            </div>
          </header>
          <section className="p-5">
            <div className="grid gap-4 md:grid-cols-3">
              <InfoCard label="Tenant" value={me.tenant?.name || "—"} />
              <InfoCard label="Role access" value={`${me.permissions.length} permissions`} />
              <InfoCard label="Status" value="Identity + tenancy active" />
            </div>
            <div className="mt-5 rounded-lg border border-slate-200 bg-white p-5">
              <h2 className="font-semibold">Phase 1</h2>
              <p className="mt-2 text-sm text-slate-600">
                Authentication, tenant isolation, custom RBAC, platform administration, impersonation and audit APIs are now wired into the foundation. Operational pages will be filled out as their phases land.
              </p>
            </div>
          </section>
        </main>
...
function PlatformHome({
  ...
  useEffect(() => {
    request<TenantSummary[]>("/platform/tenants/", {}, access).then(setTenants).catch(() => setError("Unable to load tenants."));
  }, [access]);
  ...
  async function impersonate(tenant: TenantSummary) {
    const result = await request<{ access: string }>(
      ...
      { method: "POST", body: JSON.stringify({ reason: "Support session" }) },
      access,
    );
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900">
      <header className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
        ...
        <button className="rounded border border-slate-300 px-3 py-1.5 text-sm" onClick={onLogout}>Sign out</button>
      </header>
      <section className="mx-auto max-w-6xl p-6">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-xl font-semibold">Tenants</h1>
          <span className="text-sm text-slate-500">{tenants.length} total</span>
        </div>
        {error && <p className="mb-4 text-sm text-red-700">{error}</p>}
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500"><tr><th className="px-4 py-3">ISP</th><th>Status</th><th>Currency</th><th className="px-4 text-right">Action</th></tr></thead>
            <tbody>
              {tenants.map((tenant) => (
                <tr key={tenant.id} className="border-t border-slate-100">
                  <td className="px-4 py-3"><div className="font-medium">{tenant.name}</div><div className="text-xs text-slate-500">{tenant.slug}</div></td>
                  <td>{tenant.status}</td><td>{tenant.currency}</td>
                  <td className="px-4 text-right"><button className="rounded border border-slate-300 px-3 py-1.5 disabled:opacity-50" disabled={tenant.status !== "active"} onClick={() => impersonate(tenant)}>Impersonate</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return <label className="mb-4 block"><span className="mb-1 block text-sm font-medium">{label}</span>{children}</label>;
}

function InfoCard({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-slate-200 bg-white p-4"><div className="text-xs uppercase tracking-wide text-slate-500">{label}</div><div className="mt-1 font-semibold">{value}</div></div>;
}

function CenteredMessage({ text }: { text: string }) {
  return <main className="flex min-h-screen items-center justify-center bg-slate-100 text-sm text-slate-500">{text}</main>;
}
